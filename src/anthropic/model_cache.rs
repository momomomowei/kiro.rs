...
pub fn account_supports_model(account_id: u64, requested_model: &str) -> Option<bool> {
    let cache = cache().read();
    let models = cache.account_models.get(&account_id)?;
    Some(models.iter().any(|model| model_matches(requested_model, model)))
}
